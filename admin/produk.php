							...
							<div class="card-body">
								...
								<div class="data-tables datatable-dark">
									<table id="dataTable3" class="display" style="width:100%">
										...
										<tbody>
											<?php $brgs = mysqli_query($conn, "SELECT * from kategori k, produk p where k.idkategori=p.idkategori order by idproduk ASC");
											$no = 1;
											while ($p = mysqli_fetch_array($brgs)) { ?>
												<tr>
													...
													<td nowrap>
														<button class="btn btn-sm btn-info" data-toggle="modal" data-target="#eModal<?= $p['idproduk']; ?>">Edit</button>
														<button class="btn btn-sm btn-danger" data-toggle="modal" data-target="#hModal<?= $p['idproduk']; ?>">Hapus</button>
													</td>
												</tr>
												<div id="eModal<?= $p['idproduk']; ?>" class="modal fade">
													<div class="modal-dialog">
														<div class="modal-content">
															...
															<div class="modal-body">
																<form action="editproduk.php" method="post" enctype="multipart/form-data">
																	...
																	<div class="form-group">
																		...
																		<input name="namaproduk" type="text" class="form-control" value="<?= $p['namaproduk']; ?>" required autofocus>
																	</div>
																	<div class="form-group">
																		<label>Nama Kategori</label>
																		<select name="idkategori" class="form-control">
																			<option selected>Pilih Kategori</option>
																			<?php $det = mysqli_query($conn, "select * from kategori order by namakategori ASC") or die(mysqli_error($d));
																			while ($d = mysqli_fetch_array($det)) { ?>
																				<option value="<?php echo $d['idkategori'] ?>" <?= ($p['idkategori'] == $d['idkategori']) ? ' selected' : ''; ?>><?php echo $d['namakategori'] ?></option>
																			<?php } ?>
																		</select>
																	</div>
																	<div class="form-group">
																	...
																	<div class="form-group">
																		...
																		<input name="rate" type="number" class="form-control" min="1" max="5" value="<?= $p['rate']; ?>" required>
																	</div>
																	<div class="form-group">
																		<label>Harga Sebelum Diskon</label>
																		<input name="hargabefore" type="number" class="form-control" value="<?= $p['hargabefore']; ?>">
																	</div>
																	<div class="form-group">
																		<label>Harga Setelah Diskon</label>
																		<input name="hargaafter" type="number" class="form-control" value="<?= $p['hargaafter']; ?>">
																	</div>
																	<div class="form-group">
															...
															<div class="modal-footer">
																...
																<input name="addproduct" type="submit" class="btn btn-info" value="Perbarui">
															</div>
															</form>
														</div>
													</div>
												</div>
												<div id="hModal<?= $p['idproduk']; ?>" class="modal fade">
													<div class="modal-dialog">
														<div class="modal-content">
															<div class="modal-header">
															...
															<div class="modal-footer">
																<form action="hapusproduk.php" method="post">
																	<input type="hidden" name="idproduk" value="<?= $p['idproduk']; ?>">
																	<button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
																	<input name="addproduct" type="submit" class="btn btn-danger" value="Hapus">
																</form>
															</div>
														</div>
													</div>
							...
							<form action="produk.php" method="post" enctype="multipart/form-data">
								...
								<div class="form-group">
									...
									<select name="idkategori" class="form-control">
										<option selected>Pilih Kategori</option>
										<?php
										$det = mysqli_query($conn, "select * from kategori order by namakategori ASC") or die(mysqli_error($d));
										while ($d = mysqli_fetch_array($det)) {
										?>
